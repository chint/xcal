...
<?php 
 ...
 $result = mysqli_query($bd, "SELECT * FROM `calendar`.`messages` WHERE `to_cid` = '$_SESSION[login]' AND `deleted` <> '1' ");
 if($row = mysqli_fetch_array($result)){

$result = mysqli_query($bd, "SELECT * FROM `calendar`.`messages` WHERE `to_cid` = '$_SESSION[login]' AND `deleted` <> '1' ORDER BY `id` DESC ");

  while ( $row = mysqli_fetch_array($result)) {
   

      $result1 = mysqli_query($bd, "SELECT * FROM `calendar`.`cus` WHERE `c_id` = '$row[from_cid]' ");
